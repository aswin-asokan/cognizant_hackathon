...
def preprocess_data(df, dummy_year="2025"):
    ...
    for col in ["accept_time", "delivery_time"]:
        if col in df_processed.columns:
            if "ds" in df_processed.columns:
                df_processed[col] = pd.to_datetime(
                    df_processed["ds"].astype(str) + " " + df_processed[col].astype(str).str.strip(),
                    format="%Y-%m-%d %H:%M:%S",
                    errors="coerce"
                )
            else:
                df_processed[col] = pd.to_datetime(
                    dummy_year + "-01-01 " + df_processed[col].astype(str).str.strip(),
                    format="%Y-%m-%d %H:%M:%S",
                    errors="coerce"
                )
    ...
    if set(["accept_time", "delivery_time"]).issubset(df_processed.columns):
        df_processed["delivery_delay"] = (df_processed["delivery_time"] - df_processed["accept_time"]).dt.total_seconds() / 60
        df_processed.loc[df_processed["delivery_delay"] < 0, "delivery_delay"] += 24*60
        df_processed["delivery_delay"] = df_processed["delivery_delay"].fillna(df_processed["delivery_delay"].median())
    else:
...
def detect_fraud(df, random_seed=42):
    ...
    # Fit Isolation Forest with auto contamination
...
def main():
    ...
    st.title("ShipGuard")
    st.markdown("Upload a CSV file with shipment data to detect potential fraud using Isolation Forest algorithm.")
        ...
        try:
            ...
            if st.button("🔍 Detect Fraud", type="primary"):
                with st.spinner("Processing data and detecting fraud..."):
                    ...
                    st.histogram_chart = st.bar_chart(np.histogram(fraud_scores, bins=30)[0])
